...
def append_lead_to_sheet(nome, username, link):
    try:
        ...
        logger.info(f"Lead aggiunto: {username}")
...
AFFERMATIVO = {
    ...
    "solo una", "solo due", "siuro", "siurissimo", "top",
    "👍", "✅", "💪", "🔥",
}
...
VAGO = {
    ...
    "boh", "mah", "non saprei", "non lo so bene",
...
DOMANDE_GUADAGNO = {
    "quanto si guadagna", "quanto posso guadagnare",
    "quanto posso fare", "quanti soldi", "quanto guadagni",
    "è conveniente", "vale la pena",
}

DOMANDE_FIDUCIA = {
    "è una truffa", "sembra una truffa", "non mi fido",
    "è sicuro", "è legale", "ci sono rischi",
...
DOMANDE_GIA_SENTITO = {
    ...
    "ho già provato robe simili", "è come gli altri",
}
...
OBIEZIONI_TEMPO = {
    "non ho tempo", "ho poco tempo", "sono occupato",
    "sono impegnato", "non riesco", "troppo impegnato",
}

OBIEZIONI_PENSIERO = {
    "ci penso", "magari dopo", "forse più avanti",
    "ci devo pensare", "vediamo", "non ora", "più tardi",
    "fammi pensare", "devo pensarci",
}

OBIEZIONI_SOLDI = {
    "non ho soldi", "non ho budget", "costa qualcosa",
    "quanto costa", "ci vogliono soldi", "non posso permettermi",
    "sono al verde", "ho pochi soldi",
}

OBIEZIONI_DIFFICOLTA = {
    "è complicato", "è difficile", "non sono capace",
    "non ci riesco", "non sono pratico",
    "non sono bravo con il telefono",
    "non capisco queste cose", "è troppo tecnico",
    "non so usare le app",
}

OBIEZIONI_GUADAGNO_DUBBIO = {
    "ma guadagno davvero", "funziona davvero",
    "si guadagna davvero", "è vero che si guadagna",
    "ma davvero funziona",
}
...
def check_obiezione(text: str):
    ...
    if check_set(text, DOMANDE_BOT): return "bot"
    if check_set(text, DOMANDE_FUNZIONAMENTO): return "funzionamento"
    if check_set(text, DOMANDE_GUADAGNO): return "guadagno"
    if check_set(text, DOMANDE_FIDUCIA): return "fiducia"
    if check_set(text, DOMANDE_GIA_SENTITO): return "gia_sentito"
    if check_set(text, DOMANDE_PARTNER): return "partner"
    if check_set(text, OBIEZIONI_TEMPO): return "tempo"
    if check_set(text, OBIEZIONI_PENSIERO): return "pensiero"
    if check_set(text, OBIEZIONI_SOLDI): return "soldi"
    if check_set(text, OBIEZIONI_DIFFICOLTA): return "difficolta"
    if check_set(text, OBIEZIONI_GUADAGNO_DUBBIO): return "guadagno_dubbio"
    return None
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Se non ha ancora fatto /start oppure il flusso non è iniziato
    if not step:
    ...
    if obiezione == "fiducia":
        await typing(update)
        await update.message.reply_text(f"Ti capisco {nome}, è giusto essere cauti 🙌 Nessun deposito, nessun rischio — sono app ufficiali di banche e servizi finanziari regolamentati. Luca ti mostrerà tutto in trasparenza.\n\n{domanda_corrente}")
        return

    if obiezione == "gia_sentito":
        await typing(update)
        await update.message.reply_text(f"Capito {nome}! Probabilmente hai visto cose diverse — qui non si vende nulla e non c'è nessun investimento. Luca ti fa vedere esattamente cosa fa lui, numeri alla mano 🔥\n\n{domanda_corrente}")
        return

    if obiezione == "partner":
        await typing(update)
        await update.message.reply_text(f"Certo {nome}, assolutamente! Quando vuoi Luca può spiegare tutto anche a loro 🙌\n\n{domanda_corrente}")
        return

    if obiezione == "tempo":
        await typing(update)
        await update.message.reply_text(f"Capito {nome}! Non serve molto — Luca ti segue passo per passo e ci vogliono letteralmente 10 minuti per iniziare la prima app. Si fa tutto dal telefono 📱\n\n{domanda_corrente}")
        return

    if obiezione == "pensiero":
        await typing(update)
        await update.message.reply_text(f"Certo {nome}, prenditi il tempo che ti serve! Tieni presente che i bonus hanno disponibilità limitata 🙌\n\n{domanda_corrente}")
        return

    if obiezione == "soldi":
        await typing(update)
        await update.message.reply_text(f"Buona notizia {nome} — non serve nessun deposito e nessuna spesa. Le app sono completamente gratuite, i bonus te li danno loro a te 💰\n\n{domanda_corrente}")
        return

    if obiezione == "difficolta":
        await typing(update)
        await update.message.reply_text(f"Per niente {nome}! Luca ti segue passo per passo, una app alla volta. Non serve saper fare nulla di tecnico 📱\n\n{domanda_corrente}")
        return

    if obiezione == "guadagno_dubbio":
        await typing(update)
        await update.message.reply_text(f"Sì {nome} — Luca lo fa lui stesso e lo fanno già le persone che segue. Te lo mostrerà con i numeri reali 💪\n\n{domanda_corrente}")
        return
    ...
    if step == "quali_app":
        ...
        if is_no(text):
            ...
            if "ISYB" in text_upper and "ISYBANK" not in fatte: fatte.append("ISYBANK")
