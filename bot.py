...
REGOLE:
1. Tutto gratis, zero investimenti
2. Domande fiscali/ISEE/sussidi → "Informati con il tuo commercialista"
3. Dettagli tecnici app → "Ne parlerai con Luca"
4. Tempistiche contatto → "Ti contatterà il prima possibile"
5. Mai info false
6. Tono amichevole, diretto, rassicurante
7. Rispondi SEMPRE in italiano
8. Max 2-3 righe per risposta
9. NON ripetere domande già fatte — guarda lo storico delle risposte
10. Dopo aver risposto, fai UNA SOLA domanda: quella successiva nel flusso

FLUSSO IN ORDINE:
Step 1: "Sai come funzionano i bonus app?" → se sì vai a step 2, se no spiega e vai a step 2
Step 2: "Hai già fatto qualche app in passato?"
Step 3: mostra lista app e chiedi "Hai già fatto qualcuna di queste app?"
Step 4: "C'è qualcosa che vorresti far sapere a Luca prima di iniziare?"

IMPORTANTE: Guarda attentamente le risposte già date nel contesto. Se uno step è già stato risposto, NON ripetere quella domanda — vai allo step successivo."""
...
async def ask_claude(state: dict, user_message: str, step: str) -> str:
    ...
    try:
        ...
        # Determina quale domanda fare dopo
        next_question = {
            "conosce_bonus": "Hai già fatto qualche app in passato?",
            "dopo_spiegazione": "Hai già fatto qualche app in passato?",
            "dopo_spiegazione_attesa": "Hai già fatto qualche app in passato?",
            "ha_fatto_app": "Ti manderò la lista delle app disponibili. Hai già fatto qualche app bancaria in passato?",
            "quali_app": "C'è qualcosa che vorresti far sapere a Luca prima di iniziare?",
            "messaggio_finale": "",
        }.get(step, "")
...
RISPOSTE GIÀ DATE (non ripetere queste domande!):
{storico_str}

MESSAGGIO DEL LEAD: "{user_message}"

Rispondi in modo naturale e breve (max 2-3 righe).
Dopo la risposta, fai questa domanda: {next_question}
Se il messaggio non è una domanda ma una risposta, elaborala e vai avanti nel flusso."""
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Se è una frase lunga/complessa E ha già risposto a step 1 → Claude prima delle obiezioni
    # Questo evita che obiezioni intercettino risposte che Claude dovrebbe gestire nel contesto
    is_complex = len(text.split()) > 6 and "?" not in text
    already_in_flow = step in ("ha_fatto_app", "quali_app", "messaggio_finale")
    if is_complex and already_in_flow and obiezione in (None, "funzionamento", "guadagno_totale"):
    ...
    if step == "conosce_bonus":
        # Se è una domanda o frase complessa → Claude prima
        is_question = "?" in text or len(text.split()) > 4
        if is_question and not is_yes(text) and not is_no(text):
            risposta_claude = await ask_claude(state, text, step) if ANTHROPIC_API_KEY else None
            if risposta_claude:
                await typing(update)
                await update.message.reply_text(risposta_claude)
    ...
    if step == "dopo_spiegazione":
        if check_set(text, FRUSTRAZIONI):
            ...
            # Domanda o frase complessa → Claude risponde e poi va avanti
            risposta_claude = await ask_claude(state, text, step) if ANTHROPIC_API_KEY else None
            if risposta_claude:
                await typing(update)
                await update.message.reply_text(risposta_claude)
    ...
    if step == "ha_fatto_app":
        ...
        # Se è una domanda o frase complessa (contiene ?, o è lunga) → Claude
        is_question = "?" in text or len(text.split()) > 4
        if is_question and not is_yes(text) and not is_no(text):
            risposta_claude = await ask_claude(state, text, step) if ANTHROPIC_API_KEY else None
            if risposta_claude:
    ...
    if step == "quali_app":
        # Se è una domanda complessa → Claude prima
        is_question = "?" in text or (len(text.split()) > 5 and not any(app.lower() in text.lower() for app in ["revolut", "bbva", "isybank", "buddybank", "buddy", "credit agricole", "krak"]))
        if is_question:
            risposta_claude = await ask_claude(state, text, step) if ANTHROPIC_API_KEY else None
            if risposta_claude:
                await typing(update)
    ...
    if step == "messaggio_finale":
        # Se fa una domanda invece di dare info → Claude risponde senza andare avanti
