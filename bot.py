...
async def send_summary_to_luca(context: ContextTypes.DEFAULT_TYPE, state: dict, user_obj, step_appena_completato: str):
    """Manda a Luca un riassunto aggiornato della conversazione ad ogni step completato"""
    try:
        nome = state.get("nome", "")
        username = f"@{user_obj.username}" if user_obj.username else f"ID:{user_obj.id}"
        link = f"t.me/{user_obj.username}" if user_obj.username else f"ID:{user_obj.id}"

        righe = [f"📋 AGGIORNAMENTO — {nome} ({username} → {link})\n"]

        if "conosce_bonus_risposta" in state:
            righe.append(f"❓ Bonus app: \"{state['conosce_bonus_risposta']}\"")
        if "ha_fatto_app_risposta" in state:
            righe.append(f"📱 App passate: \"{state['ha_fatto_app_risposta']}\"")
        if "quali_app_risposta" in state:
            righe.append(f"✅ App fatte: \"{state['quali_app_risposta']}\"")
        if "messaggio_preparazione" in state:
            righe.append(f"💬 Prep: \"{state['messaggio_preparazione']}\"")
...
async def send_report(context, state: dict, user_obj, tipo: str = "normale"):
    ...
    entry = {"report": report, "tipo": tipo, "time": datetime.now()}
    if tipo == "normale":
...
async def followup_4h(context: ContextTypes.DEFAULT_TYPE):
    ...
    if last and (datetime.now() - last).total_seconds() < 4 * 3600 - 60:
        return
...
async def report_24h(context: ContextTypes.DEFAULT_TYPE):
    ...
    if state_ref.get("step") == "fine":
        return
    last = state_ref.get("last_message_time")
    if last and (datetime.now() - last).total_seconds() < 24 * 3600 - 60:
        return
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "conosce_bonus":
        if is_yes(text):
            ...
            await send_summary_to_luca(context, state, user, "conosce_bonus")
        elif is_no(text):
            state["conosce_bonus_risposta"] = text
    ...
    if step == "ha_fatto_app":
        if is_yes(text) or is_no(text) or is_vague(text):
            ...
            await send_summary_to_luca(context, state, user, "ha_fatto_app")
        else:
            state["fallback_count"] = state.get("fallback_count", 0) + 1
            if state["fallback_count"] >= 2:
                paused_leads.add(user_id)
    ...
    if step == "quali_app":
        ...
        state["fallback_count"] = 0
        await send_summary_to_luca(context, state, user, "quali_app")
        await typing(update)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if pending_messages.get(user_id) is task:
